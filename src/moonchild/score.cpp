...
void score_display(VIEWPORT *player)
{
  ...
  #if 0
  {
    ...
    UINT32 total_seconds = ticks / 60;
    UINT32 hundredths = (ticks * 100 / 60) % 100;
    UINT16 secs = total_seconds % 60;
    UINT16 mins = total_seconds / 60;

    frame = anim_setsequence(orgscore, 0, SEQ_FORCE);

    frame = anim_forceframe (orgscore, mins / 10);
    frame->draw(*player->loadedmap->blitbuf, 8 + (0*40), 8);

    frame = anim_forceframe (orgscore, mins % 10);
    frame->draw(*player->loadedmap->blitbuf, 8 + (1*40), 8);


    frame = anim_forceframe (orgscore, secs / 10);
    frame->draw(*player->loadedmap->blitbuf, 24 + (2*40), 8);

    frame = anim_forceframe (orgscore, secs % 10);
    frame->draw(*player->loadedmap->blitbuf, 24 + (3*40), 8);


    frame = anim_forceframe (orgscore, hundredths / 10);
    frame->draw(*player->loadedmap->blitbuf, 40 + (4*40), 8);
...
//  if (world !=3)
    {
      frame->draw(*player->loadedmap->blitbuf,80 + (0*40) + ((sinus512[xcnt]*10)>>10) - scoreshift, (renderHeight>240)*256 + (renderHeight<=240)*16 + ((sinus512[ycnt]*10)>>10) );
    }
  ...
  frame->draw(*player->loadedmap->blitbuf,70 + (1*40) + ((sinus512[xcnt]*10)>>10) - scoreshift, (renderHeight>240)*256 + (renderHeight<=240)*16 + ((sinus512[ycnt]*10)>>10) );

  scoreposhold[holdcnt   ] = 70 + (0*40) + ((sinus512[xcnt]*10)>>10) - scoreshift + 32;
  scoreposhold[holdcnt+1 ] = (renderHeight>240)*440 + (renderHeight<=240)*200 + ((sinus512[ycnt]*10)>>10) + 8;
  holdcnt+=2;
  ...
  {  
    ...
    scoreposhold[holdcnt+1 ] = (renderHeight>240)*440 + (renderHeight<=240)*200 + ((sinus512[ycnt]*10)>>10) + scoreshift;
    holdcnt+=2;
	...
	if (world !=3)
	  {
	    frame->draw(*player->loadedmap->blitbuf,((i-2)*40) + ((sinus512[xcnt]*10)>>10)+4, (renderHeight>240)*440 + (renderHeight<=240)*200 + ((sinus512[ycnt]*10)>>10) + scoreshift );
	  }
      }
    else
      {
